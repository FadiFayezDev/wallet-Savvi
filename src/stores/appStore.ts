...
export const useAppStore = create<AppState>((set) => ({
  ...
  bootstrap: async () => {
    try {
      ...
      const locale = (settingsRow?.locale === "en" ? "en" : "ar") as
        | "ar"
        | "en";
...
export const reportService = {
  async generateMonthlyReport(monthKey: string) {
    return await runInTransaction(async (db) => {
      ...
      await runQuery(
        `DELETE FROM report_category_breakdown WHERE report_id = ?`,
        [reportId],
        db,
      );
  ...
  async getReport(monthKey: string) {
    const report = await getFirst<any>(
      `SELECT * FROM monthly_reports WHERE month_key = ?`,
      [monthKey],
    );
  ...
  async getDashboardSummary(): Promise<DashboardSummaryData> {
    ...
    const limitStatus = await getFirst<{
      is_over_limit: number;
      remaining_from_limit: number;
    }>(
