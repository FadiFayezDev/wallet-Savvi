    ...
    accountService
      ...
      .then((rows) => {
        ...
        if (rows.length > 0) {
          ...
          setSelectedAccount(
            (prev) => prev ?? defaultAccount?.id ?? rows[0].id,
          );
        }
      })
      .catch(() => setAccounts([]));
  ...
  const submit = async () => {
    ...
    if (!selectedAccount) {
      Alert.alert(
        ...
        locale === "ar"
          ? "اختيار الحساب مطلوب قبل الحفظ."
          : "Account is required before saving.",
      );
      return;
    }
    ...
    >
      ...
      <View>
        ...
        <TextInput
          ...
          placeholder={
            locale === "ar" ? "سبب العملية أو ملاحظة" : "What is this for?"
          }
          style={inputStyle}
          placeholderTextColor={theme.colors.onSurfaceVariant}
        />
