...
export default function CurrentMonthReport() {
  ...
  const grouped = useMemo(() => {
    return weekRanges.map((range) => {
      ...
      const expense = items
        .filter((tx) =>
          ["expense", "bill_payment", "work_expense"].includes(tx.kind),
        )
        .reduce((sum, tx) => sum + tx.amountAbs, 0);
      return {
        label: range.label,
  ...
  const maxDailyAbs = useMemo(() => {
    return (
      dailySeries.reduce((acc, d) => Math.max(acc, Math.abs(d.net)), 0) || 1
    );
  }, [dailySeries]);

  const maxWeekValue = useMemo(() => {
    return (
      grouped.reduce((acc, w) => Math.max(acc, w.income, w.expense), 0) || 1
    );
  }, [grouped]);
      ...
      <View style={{ gap: 16 }}>
        ...
        >
          <View style={styles.heroRow}>
            ...
            >
              <Text
                style={[styles.backText, { color: theme.colors.headerText }]}
              >
                {isAr ? "رجوع" : "Back"}
              </Text>
            </Pressable>
            ...
            >
              <Text style={[styles.liveText, { color: theme.colors.success }]}>
                {isAr ? "LIVE" : "LIVE"}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.heroStats}>
            ...
            >
              <Text
                style={[
                  styles.kpiLabel,
                  { color: withAlpha(theme.colors.headerText, 0.7) },
                ]}
              >
                {isAr ? "الدخل" : "Income"}
              </Text>
              <Text
                style={[styles.kpiValue, { color: theme.colors.headerText }]}
              >
            ...
            <View
              style={[
                styles.kpiCard,
                { backgroundColor: withAlpha(theme.colors.error, 0.18) },
              ]}
            >
              <Text
                style={[
            ...
            <View
              style={[
                styles.kpiCard,
                { backgroundColor: withAlpha(theme.colors.info, 0.2) },
              ]}
            >
              <Text
                style={[
          ...
          {loading ? (
            ...
            <View style={styles.sparkRow}>
              {dailySeries.map((d) => {
                ...
                const color =
                  d.net >= 0 ? theme.colors.success : theme.colors.error;
                return (
          ...
          <View style={{ gap: 10 }}>
            {grouped.map((week) => (
              <View key={week.label} style={styles.weekRow}>
                <Text
                  style={[styles.weekLabel, { color: theme.colors.onSurface }]}
                >
                  ...
                  >
                    <View
                      style={[
                        styles.weekFill,
                        {
                          width: `${Math.max(5, (week.income / maxWeekValue) * 100)}%`,
                  ...
                  >
                    <View
                      style={[
                        styles.weekFill,
                        {
                          width: `${Math.max(5, (week.expense / maxWeekValue) * 100)}%`,
                ...
                <Text
                  style={{
                    color:
                      week.net >= 0 ? theme.colors.success : theme.colors.error,
                    fontWeight: "700",
                  }}
                >
        ...
        >
          ...
          <View style={styles.signalGrid}>
            ...
            >
              ...
              <Text
                style={[styles.signalValue, { color: theme.colors.onSurface }]}
              >
                {topExpense
                  ? isAr
                    ? topExpense.nameAr
          ...
          <View style={{ marginTop: 8, gap: 10 }}>
            {grouped.map((week) => (
              ...
              >
                ...
                >
                  {isAr ? "دخل" : "Income"}:{" "}
                  {formatMoney(week.income, localeKey, currency)} •{" "}
                  {isAr ? "مصروف" : "Expense"}:{" "}
                  {formatMoney(week.expense, localeKey, currency)}
                </Text>
                {week.items.length === 0 ? (
                  <Text style={{ color: theme.colors.onSurfaceVariant }}>
