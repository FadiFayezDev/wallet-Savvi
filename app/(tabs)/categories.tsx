...
// ── الشاشة الرئيسية ──────────────────────────────────────────────
interface CategoriesTabScreenProps {
  initialType?: "expense" | "income";
}

export default function CategoriesTabScreen({
  initialType,
}: CategoriesTabScreenProps = {}) {
  ...
  const switchTab = useCallback(
    (tab: CategoryTab, opts: { reset?: boolean } = {}) => {
  ...
  // ตั้งค่า activeTab เมื่อ initialType เปลี่ยนไป
  useEffect(() => {
    if (initialType) {
      switchTab(initialType, { reset: false });
    }
  }, [initialType, switchTab]);
    ...
    <MaterialScreen layout="tab">
      <View style={styles.titleRow}>
        ...
        <Surface
          ...
          style={[
            styles.countBadge,
            { backgroundColor: withAlpha(activeColor, 0.14) },
          ]}
        >
          <Text
            variant="labelLarge"
            style={[styles.countText, { color: activeColor }]}
          >
            {filtered.length}
          </Text>
        </Surface>
