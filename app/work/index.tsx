...
import {
  TimePickerField,
  formatTimeLabel,
} from "@/src/components/common/TimePickerField";
...
export default function WorkScreen() {
  ...
  const load = useCallback(async () => {
    const [scheduleRows, expenseRows, logRows, todayLogRow, accountRows] =
      await Promise.all([
    ...
    setSelectedAccountId(
      (prev) => prev ?? defaultAccount?.id ?? accountRows[0]?.id ?? null,
    );
  }, [todayIso]);
  ...
  const formatTime = (value: string | null) =>
    value ? formatTimeLabel(value, timeFormat, locale) : "-";
  ...
  const onApplyStandardToday = async () => {
    ...
    const result = await workService.logStandardWorkDay(
      todayIso,
  ...
  const onSkipToday = async () => {
    ...
    Alert.alert(
      ...
      isAr
        ? "لن يتم خصم مصاريف العمل لهذا اليوم."
        : "No work expenses will be deducted today.",
      [
  ...
  const onSaveSchedule = async () => {
    ...
    Alert.alert(
      isAr ? "تم الحفظ" : "Saved",
      isAr ? "تم حفظ إعدادات اليوم." : "Day settings have been saved.",
    );
  };
  ...
  const onLogCustomDay = async () => {
    ...
    try {
      ...
      Alert.alert(
        isAr ? "تم الحفظ" : "Saved",
        isAr ? "تم تسجيل اليوم." : "Custom day logged.",
      );
    } catch (err) {
  ...
  const onDeleteLog = async (log: WorkDayLog) => {
    const ok = await confirmAction({
      ...
      message: isAr
        ? "سيتم حذف هذا السجل وإلغاء خصم مصاريفه إن وجدت."
        : "This log will be removed and its expense reversed if possible.",
        ...
        >
          <View style={styles.heroTop}>
            <Pressable
              ...
              style={[
                styles.iconChip,
                { backgroundColor: withAlpha(theme.colors.headerText, 0.12) },
              ]}
            >
              <IconButton
            ...
            <View style={{ flex: 1 }}>
              <Text
                style={[styles.heroTitle, { color: theme.colors.headerText }]}
              >
              ...
              <Text
                style={[
                  styles.heroSubtitle,
                  { color: withAlpha(theme.colors.headerText, 0.65) },
                ]}
              >
                {isAr
                  ? "تحكم ذكي في يومك — خصم مرن عند الحاجة"
                  : "Smart control of your work day expenses"}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.heroRow}>
            <View
              style={[
                styles.statusPill,
                { backgroundColor: withAlpha(todayStatus.color, 0.18) },
              ]}
            >
            ...
            >
              <Text
                style={[styles.totalText, { color: theme.colors.headerText }]}
              >
              ...
              <Text
                style={[
                  styles.totalSub,
                  { color: withAlpha(theme.colors.headerText, 0.6) },
                ]}
              >
            ...
            >
                ...
                <Pressable
                  ...
                  style={[
                    styles.actionBtn,
                    {
                      ...
                      opacity: !isTodayWorkDay || todayLog ? 0.6 : 1,
                    },
                  ]}
                >
                  <Text
              ...
              <View style={styles.infoGrid}>
                ...
                >
                  ...
                  >
                    {formatTime(todaySchedule?.startTime ?? null)} -{" "}
                    {formatTime(todaySchedule?.endTime ?? null)}
                  </Text>
                </View>
                <View
                  style={[
                ...
                <Pressable
                  onPress={async () => {
                    ...
                    const ok = await confirmAction({
                      ...
                      message: isAr
                        ? goingWorkDay
                          ? "سيتم تفعيل هذا اليوم كـ يوم عمل."
                          : "سيتم اعتبار هذا اليوم إجازة."
                        : goingWorkDay
                          ? "This day will be marked as a work day."
                          : "This day will be treated as off.",
                  ...
                  style={[
                    styles.togglePill,
                    {
                      backgroundColor: selectedSchedule?.isWorkDay
                        ? theme.colors.success
                        : theme.colors.surfaceVariant,
                    },
                  ]}
                >
                  <Text
                    style={[
                      styles.toggleText,
                      {
                        color: selectedSchedule?.isWorkDay
        ...
        {activeSection === "expenses" && (
          <View style={{ gap: 12 }}>
            ...
            >
              ...
              >
                <Text
                  style={[styles.actionBtnText, { color: theme.colors.onInfo }]}
                >
                  {isAr ? "إضافة" : "Add"}
                </Text>
              </Pressable>
                      ...
                      <Pressable
                        onPress={async () => {
                          ...
                          const ok = await confirmAction({
                            ...
                            message: isAr
                              ? goingActive
                                ? "سيتم احتساب هذا المصروف ضمن خصم اليوم."
                                : "لن يتم احتساب هذا المصروف ضمن خصم اليوم."
                              : goingActive
                                ? "This expense will be included in daily deduction."
                                : "This expense will be excluded from daily deduction.",
                            confirmText: isAr ? "تأكيد" : "Confirm",
                            cancelText: isAr ? "إلغاء" : "Cancel",
                          });
                          if (!ok) return;
                          await workService.toggleExpenseActive(
                            row.id,
                            !row.isActive,
                          );
                          await load();
