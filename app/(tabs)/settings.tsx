...
const lockMethods: ("none" | "pin" | "biometric")[] = [
  "none",
  "pin",
  "biometric",
];
...
export default function SettingsTab() {
  ...
  useEffect(() => {
    if (!settings) return;
    setDailyLimitInput(
      settings.dailyLimit == null ? "" : String(settings.dailyLimit),
    );
  }, [settings?.dailyLimit]);
    ...
    <MaterialScreen title={pageTitle} layout="tab">
      ...
      <SettingsCard>
        <List.Section style={styles.listSection}>
          ...
          <ComboSelect
            ...
            onChange={(fmt) => {
              patchSettings({ timeFormat: fmt as "12h" | "24h" }).catch(
                () => undefined,
              );
            }}
            listRow={{
        ...
        <List.Section style={styles.listSection}>
          ...
          <ComboSelect
            ...
            listRow={{
              ...
              description: isAr
                ? "Material You أو ثابت أو مخصص"
                : "Dynamic color or fixed palette",
              icon: "palette-swatch",
            }}
          />
          <Divider />
          <List.Item
            ...
            description={
              isAr ? "إنشاء، استيراد، تصدير" : "Create, import, export"
            }
          ...
          <List.Item
            ...
            description={
              isAr ? "ثيم كامل من لوحة ألوان" : "Full palette themes"
            }
      ...
      <SettingsCard>
        <Card.Content style={styles.cardPad}>
          ...
          >
            {isAr
              ? "تنبيه عند التجاوز — لا يتم الحجب"
              : "Alert when exceeded — nothing is blocked"}
          </Text>
          <View style={{ marginTop: 12, marginBottom: 10 }}>
            <CalculatorField
              ...
              hint={
                isAr
                  ? "اختياري — اتركه فارغًا لإلغاء الحد"
                  : "Optional — empty removes the limit"
              }
          ...
          <ComboSelect
            ...
            onChange={async (method) => {
              ...
              if (method === "biometric") {
                ...
                if (!canUse) {
                  Alert.alert(
                    isAr ? "البصمة غير متاحة" : "Biometric unavailable",
                  );
                  return;
                }
              }
              await patchSettings({
                lockMethod: method as "none" | "pin" | "biometric",
              });
            }}
            listRow={{
            ...
            <Button
              ...
              onPress={async () => {
                const ok = await confirmAction({
                  ...
                  message: isAr
                    ? "سيتم تعطيل الحماية بالـ PIN."
                    : "PIN protection will be disabled.",
      ...
      <SettingsCard>
        <List.Section style={styles.listSection}>
          <List.Item
            ...
            left={(props) => (
              <List.Icon {...props} icon="receipt-text-outline" />
            )}
            right={() => (
              <Switch
                value={settings.notifyBillsEnabled}
                onValueChange={toggleNotifyBills}
              />
            )}
          />
          <Divider />
          <List.Item
            ...
            right={() => (
              <Switch
                value={settings.notifyWorkEnabled}
                onValueChange={toggleNotifyWork}
              />
            )}
          />
        </List.Section>
